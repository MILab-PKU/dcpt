...
int32_t best_block_dim(int32_t feat_dim){
    int32_t best_dim;
    if (feat_dim < 384){
        best_dim = 64;
    }
...
__global__ void roll_and_window_partition_forward_cuda_kernel(
    ...
    int32_t offset;
    for (int32_t i = index; i < C; i += blockDim.x) {
        offset = ((blockIdx.z * gridDim.y + blockIdx.y) * gridDim.x + blockIdx.x) * C + i; // C = blocksize
        int32_t input_offset = blockIdx.z / (nH * nW) * H * W * C +
...
at::Tensor roll_and_window_partition_forward_cuda(
    ...
    int32_t nH = H / window_size;
    int32_t nW = W / window_size;

    dim3 grid(window_size, window_size, B * nH * nW);
    //dim3 block((C + 31) / 32 * 32);
    int32_t blocknum = best_block_dim(C);
    dim3 block(blocknum);
