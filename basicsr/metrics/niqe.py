...
def calculate_niqe(img, crop_border, input_order="BCHW", convert_to="y", **kwargs):
    """Calculate NIQE (Natural Image Quality Evaluator) metric.
    ...
    for i in range(img.shape[0]):
        niqe_results.append(
            niqe(img[i, ...], mu_pris_param, cov_pris_param, gaussian_window)
        )

    return np.array(niqe_results).mean()
